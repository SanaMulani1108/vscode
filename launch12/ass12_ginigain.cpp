...
struct AttributeResults {
    ...
    AttributeResults(int idx, double pEntropy, double cEntropy, double iGain, double pGini, double cGini)
        : attributeIndex(idx), parentEntropy(pEntropy), childEntropy(cEntropy), infoGain(iGain), parentGini(pGini), childGini(cGini) {}
};

double log2(double value) {
    return (value == 0) ? 0 : log(value) / log(2);
}
...
int main() {
    ...
    cout << "Parent Information Gain (Entropy): " << parentEntropy << endl;
    ...
    for (int attributeIndex = 0; attributeIndex < 4; attributeIndex++) {
        ...
        double childEntropy = 0, childGini = 0;

        cout << left << setw(15) << "Attribute Value" << setw(5) << "Play" << setw(10) << "Don't Play" 
             << setw(6) << "Total" << setw(10) << "Entropy" << setw(10) << "Gini Index" << endl;
        cout << "--------------------------------------------------------------------------" << endl;
        ...
        for (const auto &entry : childCount) {
            ...
            double entropy = calculateEntropy(playCount, childTotal);
            ...
            cout << setw(15) << key << setw(5) << playCount << setw(10) << dontPlayCount
                 << setw(6) << childTotal << setw(10) << entropy << setw(10) << gini << endl;
        }
        ...
        resultsList.push_back(AttributeResults(attributeIndex, parentEntropy, childEntropy, parentEntropy - childEntropy, parentGini, childGini));
    }

    auto bestAttribute = max_element(resultsList.begin(), resultsList.end(), 
    ...
    if (bestAttribute != resultsList.end()) {
        cout << "Best splitting criterion is Attribute " << bestAttribute->attributeIndex 
             << " with Information Gain: " << bestAttribute->infoGain << endl;
    }
